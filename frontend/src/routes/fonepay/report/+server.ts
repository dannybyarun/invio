...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!dates.length || dates.length > 31) {
      return json(
        { error: "Select a date range of 1 to 31 days" },
        { status: 400 },
      );
    }
    return json({
      results: await fetchFonepayReports(locals.authHeader, dates),
    });
  } catch (error) {
    console.error("Fonepay report request failed", error);
    const message =
      error instanceof FonepayUpstreamError
        ? error.message
        : "Unable to load Fonepay transactions";
    return json(
      { error: message },
      { status: error instanceof FonepayUpstreamError ? error.status : 502 },
    );
  }
};
